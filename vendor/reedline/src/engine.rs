...
pub struct Reedline {
    ...
    pre_edit_hook: Option<Box<dyn FnMut(&Event, &str, usize) -> Option<ReedlineEvent> + Send>>,
...
impl Reedline {
    ...
    /// Set a pre-edit hook that is called before each key event is processed.
    /// The hook receives the event, the current buffer content, and cursor position.
    /// If it returns `Some(ReedlineEvent)`, that event is used instead of the
    /// normal key-to-event mapping.
    #[must_use]
    pub fn with_pre_edit_hook(
        mut self,
        hook: Box<dyn FnMut(&Event, &str, usize) -> Option<ReedlineEvent> + Send>,
    ) -> Self {
        self.pre_edit_hook = Some(hook);
        self
    }
    ...
    fn read_line_helper(&mut self, prompt: &dyn Prompt) -> Result<Signal> {
        ...
        loop {
            ...
            // Pre-edit hook: if set, run it on each event before normal mapping.
            if let Some(ref mut hook) = self.pre_edit_hook {
                let buffer = self.editor.get_buffer().to_string();
                let cursor = 0; // hook receives buffer snapshot; position not critical
                for event in &events {
                    if let Some(override_event) = hook(event, &buffer, cursor) {
                        match override_event {
                            ReedlineEvent::Edit(edit) => edits.extend(edit),
                            other => {
                                let taken = std::mem::take(&mut edits);
                                if !taken.is_empty() {
                                    reedline_events.push(ReedlineEvent::Edit(taken));
                                }
                                reedline_events.push(other);
                            }
                        }
                    } else if let Ok(ev) = ReedlineRawEvent::try_from(event.clone()) {
                        match self.edit_mode.parse_event(ev) {
                            ReedlineEvent::Edit(edit) => edits.extend(edit),
